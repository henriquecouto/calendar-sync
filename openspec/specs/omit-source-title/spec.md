...
Privacy-mode toggle on each sync profile that prevents the source event's original title from appearing in the target event's description, replacing it with a non-reversible SHA256 fingerprint so the sync engine can still detect title changes. Preserves all other sync behavior (sync marker, copy-description / copy-location toggles).
...
- **AND** the SHA256 of "Doctor Appointment" SHALL appear as the first line
