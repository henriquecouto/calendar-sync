...
### Requirement: Global sync enabled state
**Reason**: Sync enabled/disabled is now per-profile instead of global. A global master toggle would create ambiguity.
**Migration**: The old global `sync_enabled` SharedPreferences key SHALL be migrated into the default profile's enabled flag during upgrade.
