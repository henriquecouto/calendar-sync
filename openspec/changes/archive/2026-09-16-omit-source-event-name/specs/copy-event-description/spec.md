...
#### Scenario: Copy description with omit source title both enabled

- **WHEN** a profile has `copyDescription: true` and `omitSourceTitle: true`
- **AND** the source event has title "Doctor Appointment" and description "Meeting agenda: Q3 planning"
- **THEN** the target event description SHALL be:
  ```
  Meeting agenda: Q3 planning
...
- **AND** the SHA256 fingerprint of "Doctor Appointment" SHALL appear as the title line
- **AND** the sync marker SHALL still be present
...
The profile create/edit form SHALL reorganize into Basic and Advanced sections. The Advanced section SHALL be collapsed by default and SHALL contain the sync event name field, fallback interval dropdown, copy location toggle, copy description toggle, and omit source title toggle.

#### Scenario: Copy toggles appear in Advanced section

- **WHEN** the user opens the profile create or edit form
- **THEN** the Advanced section SHALL contain "Copy location", "Copy description", and "Omit source event title" toggles
- **AND** all toggles SHALL default to off for new profiles
- **AND** all toggles SHALL reflect persisted values for existing profiles
...
- **THEN** the section SHALL expand to show sync event name, fallback interval, copy location, copy description, and omit source title